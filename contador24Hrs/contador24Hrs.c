...
void main() {
    ...
    intcon = 0;//desliga todas as interrrup??es
    tmr0ie_bit = 1;//liga a interrup??o do tmr0
    int0ie_bit = 1;//liga interrupcao do pino RB0
...
void setup_inicial(){
    ...
    trisa = 0;
    portb = 0;
    trisb = 7;//rb1 e rb2 como entrada
    portc = 0;
    trisc = 0;
    portd = 0;
    trisd = 0;
...
void escreve_display(){
    ...
    rb4_bit = 0;//desliga a unidade
    portd = decodifica(dezena);
    rb5_bit = 1;//liga a dezena
    delay_ms(1);

    rb5_bit = 0;//desliga a dezena
    portd = decodifica(centena);
    if(conta_interrupcao > 15) portd = portd + 128;
    rb6_bit = 1;//liga a centena
    delay_ms(1);

    rb6_bit = 0;//desliga a centena
...
void interrupt(){
    ...
    if(int0if_bit == 1){//ocorreu emergencia
        int0if_bit = 0;//desliga o sinalizador de emergencia
        unidade = 0;
        dezena = 0;
        centena = 0;
        milhar = 0;
