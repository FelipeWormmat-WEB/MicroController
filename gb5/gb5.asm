...
;gb5.c,39 :: 		PORTD = segment_codes[digit]; // Envia o código do dígito para os segmentos
...
L_displayNumber1:
;gb5.c,46 :: 		Delay_ms(200); // Breve atraso para estabilizar a exibição
	MOVLW       6
	MOVWF       R11, 0
	MOVLW       19
	MOVWF       R12, 0
...
;gb5.c,67 :: 		TRISD = 0;          // Configura PORTD como saída
	CLRF        TRISD+0 
;gb5.c,68 :: 		TRISB4_bit = 0;     // Configura RB4 (unidade) como saída
	BCF         TRISB4_bit+0, BitPos(TRISB4_bit+0) 
;gb5.c,69 :: 		TRISB5_bit = 0;     // Configura RB5 (dezena) como saída
	BCF         TRISB5_bit+0, BitPos(TRISB5_bit+0) 
;gb5.c,70 :: 		TRISB6_bit = 0;     // Configura RB6 (centena) como saída
	BCF         TRISB6_bit+0, BitPos(TRISB6_bit+0) 
;gb5.c,71 :: 		TRISB7_bit = 0;     // Configura RB7 (milhar) como saída
...
;gb5.c,99 :: 		displayNumber(7, 4); // Unidade
	...
	MOVWF       FARG_displayNumber_position+0 
	CALL        _displayNumber+0, 0
;gb5.c,101 :: 		Delay_ms(200); // Pequeno atraso para evitar múltiplas detecções
	MOVLW       6
	MOVWF       R11, 0
	MOVLW       19
	MOVWF       R12, 0
...
L_main24:
	...
	NOP
	NOP
;gb5.c,109 :: 		if (RB2_bit == 1) {    // Confirma botão pressionado
	BTFSS       RB2_bit+0, BitPos(RB2_bit+0) 
	GOTO        L_main25
...
L_main34:
	...
	NOP
	NOP
;gb5.c,136 :: 		if (RB3_bit == 1) {    // Confirma botão pressionado
	BTFSS       RB3_bit+0, BitPos(RB3_bit+0) 
	GOTO        L_main35
